...
class processuploadDire {
    ...
    /*   public static function Image2data( $searchFile, $rec, $itemname ) {
        global $eol, $errorBeg, $errorEnd;
        // itemname is copyright, keywords
        // assume item in image is correct, except if blank
        $msg = "";
        $basename = $rec[ "filename" ] . "_cr.jpg";
        $fileItem = readexifItem( $searchFile, $itemname, $msg );
        $cnt = $wpdbExtra>update( $rrw_photos, array( $itemname => $fileItem ),
            array( "filename" => $basename ) );
        $comment = "$basename -- $itemname -- NULL -> $fileItem";
        $msg .= rrwUtil::InsertIntoHistory( $itemname, $comment );
        return $msg;
    }

    public static function ImageXdata( $searchFile, $rec, $itemname ) {
        global $eol, $errorBeg, $errorEnd;
        global $wpdbExtra, $rrw_photos;
        // item is photodate,height, width
        // assume item in image is correct, except if blank
        $msg = "";
        $dataItem = $rec[ $itemname ];
        $basename = $rec[ "filename" ] . "_cr.jpg";;
        $fileItem = readexifItem( $searchFile, $itemname, $msg );
        if ( empty( $fileItem ) ) { //
            $msg .= pushtoimage( $searchFile, $itemname, $dataItem );
        } else {
            $cnt = $wpdbExtra->update( $rrw_photos,
                array( $itemname => $fileItem ),
                array( "filename" => $basename ) );
            $comment = "$basename -- $itemname -- NULL -> $fileItem";
            $msg .= rrwUtil::InsertIntoHistory( $itemname, $comment );
        }
...
*/
    private static function MakeImakeImages( $sourceFile, $photographer ) {
        ...
        list( $img_tmb, $w_tmb, $h_tmb, $msgTmp ) =
            self::resizeImage( $img_src, $w_src, $h_src, $w_tmb, $h_tmb, 0 );
        $msg .= $msgTmp;
        ...
        if ( $h_src > $maxHeight ) {
            ...
            list( $img_copyright, $w_cr, $h_cr, $msgTmp ) =
                self::resizeImage( $img_src, $w_src, $h_src, $w_cr, $h_cr, 0 );
            $msg .= $msgTmp;
        ...
        if ( false ) {
            ...
            if ( !self::enoughmem( $h_high, $w_src ) ) {
