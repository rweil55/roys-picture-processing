    ...
    static public function administrationPicures($attr)
    {
        ...
        try {
            ...
            if (rrwUtil::AllowedToEdit(
                "see complete administratin display",
                "",
                false
            ))
            ...
            $msg .= self::SQLcount(
                "~load before $loadDate and like -s",
...
<form method='post' action='/fix?task=deletephoto' >
   ...
   ";
    ...
    private static function ColumnOne($loadDate)
    {
        ...
        $msg .= self::SQLcount(
            "~ photos rejected/duplicates",
            "not photostatus = 'use'"
        );
        $msg .= self::EmptyCount("direonp", "source file");
        $msg .= self::SQLcount(
            "~with load dates before $loadDate",
            "uploaddate < '$loadDate' "
        );
        $msg .= self::SQLcount(
            "~with load dates after $loadDate'",
            "uploaddate >= '$loadDate' "
        );
