...
class freewheeling_fixit
{
        ...
        try {
            ...
            switch ($task) {
                ...
                case "removecreditline":
                    $msg .= freewheeling_fixit::removeCreditline($attr);
                    break;
                case "test":
   ...
   <strong>File consistency</strong>$eol
   ...
   <strong>keywords</strong>$eol
    ...
    private static function AddCapacity($attr)
    {
        ...
        $userName = rrwParam::String("user", $attr);
        $remove = rrwParam::String("remove", $attr, false);
        if (empty($userName))
    ...
    private static function extractexif()
    {
        ...
        foreach ($recs as $rec) {
            ...
            if (!is_array($exif)) {
                ...
                // $msg .= "$exifJson $eol";
                $msg .= $wpdbExtra->update($rrw_photos, $update, $where);
            }
        }
        return $msg;
    }
    ...
    private static function compareLists(
        $name1,
        $list1,
        $name2,
        $list2,
        ...
        $msg .= "Found $cntFound entries$eol";
        return $msg;
    } // end coparelists
    /*
        return $msg;
...
*/
    ...
    private static function linkTo127upload($sourcefile)
    {
        ...
        $unbold = "</strong>";
        if (
            false !== strpos($sourcefile, "contest") ||
    ...
    private static function reload($attr)
    {
        ...
        $msg = "";
        $debug = false;
        $fullFilename = rrwParam::String("fullfilename");
    ...
    private static function removeCreditline($attr)
    {
        ...
        $files = rrwUtil::getDirList($photoPath, true);
        $msg .= "<table>\n";
        $cntDire = 0;
    ...
    private static function displayExif($attr)
    {
        ...
        $msg .= "<table>" . rrwFormat::headerRow($file1, $file2) .
            rrwFormat::CellRow($display1, $display2) . "</table> $eol<img src='$file2' />";
        return $msg;
    }
    /*
    ...
    */
    ...
    private static function addList()
    {
        ...
        $msg .= self::rrwFormatDisplayPhotos(
            $sql,
            "photos that might be uploaded",
            20
        );
        return $msg;
    }
    ...
    private static function DeleteOnePhotoname($photoname, $why)
    {
        ...
        $debug = true;
        if (empty($photoname)) {
            $photoname = rrwParam::String("photoname");
            if (empty($photoname)) {
    ...
    private static function keywordDups()
    {
        ...
        $msg .= freewheeling_fixit::rrwFormatDisplayPhotos(
            $sqlKeywordDups,
    ...
    private static function listPhotog()
    {
        ...
        $msg .= freewheeling_fixit::rrwFormatDisplayPhotos(
            $sql,
            "photos created to the photographer $photog ",
            5000
        );
        return $msg;
    }
    ...
    private static function byDate()
    { //get collection of photos between two dates
        global $eol;
    ...
    public static function fileLike($attr)
    {
        ...
        if (strpos($host, "dev") !== false)
            ...
            $dev = "&dev=0";
        $partial = rrwParam::String("partial", $attr);
        $photoname = rrwParam::String("photoname", $attr);
    ...
    private static function sourcePush($attr = "")
    {
        ...
        $msg = "";
        $photoname = rrwParam::String("photoname", $attr);
        $sourcefullname = rrwParam::String("sourcefullname", $attr);
        if (empty($photoname) || empty($sourcefullname))
            throw new Exception("$msg $errorBeg
                    E#119 missing photoname or sourceFullPath dire $errorEnd");
    ...
    private static function listing()
    {
        ...
        $limitin = rrwParam::Number("limit");
        $startAt = rrwParam::Number("startat");
        ...
        $msg .= freewheeling_fixit::rrwFormatDisplayPhotos(
            $sql,
            "photos wih $description",
            $startAt,
            $urlNext
        );
        return $msg;
    }
    private static function rrwFormatDisplayPhotos(
        $sql,
        $description,
        $startAt = 0,
        $urlNext = ""
        ...
        try {
            ...
            if (0 == $missngsourceCnt)
                return $msg . "$errorEnd $missngsourceCnt records found for $errorEnd $sql $eol" .
                    rrwFormat::backtrace("");
            $color = rrwUtil::colorSwap();
            $msg .= "$eol <table><tr> \n";
                ...
                foreach ($recset as $name => $valu) {
                    ...
                    switch (strToLower($name)) {
                        ...
                        case "aspect":
                            ...
                            break;
                        case "search":
                            $photonameEncoded = urlencode($name);
    ...
    public static function updateRename($photoname = "", $newname = "")
    {
        ...
        //  ---------------------------------------------- move three files
        $msg .= self::checkAndRename(
            "$photoPath/$photoname" . ".jpg",
            "$photoPath/$newname" . ".jpg"
        );
        $msg .= self::checkAndRename(
            "$photoPath/$photoname" . "_cr.jpg",
            "$photoPath/$newname" . "_cr.jpg"
        );
        $msg .= self::checkAndRename(
            "$thumbPath/$photoname" . "_tmb.jpg",
            "$thumbPath/$newname" . "_tmb.jpg"
        );
        $msg .= self::checkAndRename(
            "$highresPath/$photoname" . ".jpg",
        ...
        if (0 != $wpdbExtra->num_rows)
            $msg .= "$errorBeg E#131 file $newname is already in the photo table,
...
<input type='submit' name='submit' id='submit' value='Search any' />
    ...
    private static function forceOneExif()
    {
        ...
        $msg .= self::fixAssumeExifCorrect($recs[0]);
        return $msg;
    }
    ...
    private static function forceDatabse2matchexif()
    {
        ...
        try {
            ...
            $cntRecs = 0;
            foreach ($recs as $rec) {
                $msg .= self::fixAssumeExifCorrect($rec);
    ...
    private static function fixAssume($rec, $exifCorrect)
    {
        ...
        try {
            ...
            $databaseKeyword = freeWheeling_DisplayOne::keywordsDisplay($photoname);
            ...
            if (empty($fileExif) || !is_array($fileExif))
                throw new Exception("$errorBeg #185 Fetch of exif from $fullFile failed $errorEnd");
            ...
            if (empty($fileArtist) && empty($databasePhotographer)); // do nothing
            elseif (empty($fileArtist) && !empty($databasePhotographer)) {
                $msg .= rrwExif::pushToImage(
                    $photoname,
                    "Artist",
                    $dataPotogrpherWeb
                );
            ...
            if (empty($FileDateTime) && empty($datebasePhotoDate)) {
                if ($debugDate) $msg .= "I#112 no dates available $eol ";
                // do nothing
            } elseif (empty($FileDateTime) && !empty($datebasePhotoDate)) {
                if ($debugDate) $msg .= "I#113 no photo date,but have database $datebasePhotoDate $eol ";
                if (strpos($datebasePhotoDate, "1800") !== false) {
                    if ($debugDate) $msg .= "I#114 a real date, pudh it to photo $eol";
                    $msg .= rrwExif::pushToImage($photoname, "DateTimeOriginal", $datebasePhotoDate);
                }
                $FileDateTime = $datebasePhotoDate;
            } elseif (!empty($FileDateTime) && empty($datebasePhotoDate)) {
                if ($debugDate) $msg .= "I#115 have photo $FileDateTime,but have database date $eol ";
                ...
                if (abs($dateDiff->days) > 1) { // more than one day difference
                    if ($debugDate) $msg .= "I#116  photo $FileDateTime, not equal database $datebasePhotoDate $eol ";
                    if ($exifCorrect) {
                        if ($debugDate) $msg .= "I#117 update database $eol";
                        $sqlUpdate["PhotoDate"] = $fileArtist;
                        $datebasePhotoDate = $fileArtist;
                    } else {
                        if ($debugDate) $msg .= "I#118 update database $eol";
                        $msg .= rrwExif::pushToImage($photoname, "DateTimeOriginal", $datebasePhotoDate);
            ...
            $fileKeywords = ""; //  self::getPhotoKeywords($fileExif);
            $debugKeywords = false;
            $keywordname = "0x9c9e";
            $keywordname = "tags";
            $keywordname = "Tags";
            ...
            if (empty($fileKeywords) && empty($databaseKeyword)); // do nothing
