...
class phpExifWriter extends phpExifReader
{
    ...
    function modifyExifDetails()
    {
        ...
        foreach ($this->ImageInfo as $tag => $val) {
            ...
            if (strncmp("0x", $tag, 2)) {   // is it a hex number?
                // format
                $fmt = $this->getFormat($tag);
                if ($fmt == -1) continue;       // not a vaild one
                $tmpTag = hexdec($tag);
                // tag
    ...
    function writeImage($file)
    {
        ...
        if (empty($file)) {
            $this->errno = 3;
            $this->errstr = "writeImage was not provided a File name!";
            throw new Exception("writeImage was not provided a File name!");
        }
        $fp = fopen($file, "wb");
